"""Padel compositor — thin per-sport wrapper over sports_utils.py.

Handles padel-specific defaults and extension points without duplicating
the crop logic, which lives entirely in sports_utils.py.

Default layout: sports_center_crop
  Padel courts are enclosed (glass walls) and filmed from a fixed high camera
  above/behind the court. The action is spatially concentrated on a smaller
  court than tennis, making center crop the best default.

Extension points (add here without touching other sports):
  - Glass wall safe-zone (avoid cropping into wall reflection artifacts)
  - Net position crop bias for overhead camera angle
"""
...
def process_sports_padel(
    ...
    """Compose a padel clip into a silent 9:16 vertical composite.

    Delegates all crop logic to sports_utils.process_sports(). Padel-specific
    logic (e.g. glass wall safe zones, net position bias) can be added here
    before the delegate call without affecting tennis, football, or future sports.
    ...
    Args:
        ...
        config:           Full pipeline configuration dict (already overlaid
                          with sports_* and sports_padel_* sections).
